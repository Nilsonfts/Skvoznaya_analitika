...
def calculate_seasonal_coefficient(month: int) -> float:
    """
    Расчет сезонного коэффициента для салона красоты
    ...
    """
    # Сезонность для салона красоты (примерные коэффициенты)
    seasonal_coefficients = {
        1: 0.9,   # Январь - спад после праздников
        2: 0.95,  # Февраль - День Святого Валентина
        3: 1.1,   # Март - подготовка к весне, 8 марта
        4: 1.05,  # Апрель - весенние процедуры
        5: 1.15,  # Май - подготовка к лету
        6: 1.1,   # Июнь - свадебный сезон
        7: 1.0,   # Июль - отпуска
        8: 0.95,  # Август - отпуска
        9: 1.05,  # Сентябрь - возвращение из отпусков
        10: 1.0,  # Октябрь - стабильный период
        11: 1.1,  # Ноябрь - подготовка к праздникам
        12: 1.2   # Декабрь - новогодние корпоративы
    }
